...
/* ===== RWD V9 RECOVERY MASTER STABILIZER =====
   This is NOT a redesign. It preserves the full existing app and only:
   1) guarantees Supabase anon auth headers exist,
   2) prevents raw JSON dumps from being shown when possible,
   3) bumps the build marker for cache reset.
*/
window.RWD_RECOVERY_VERSION = "V9 Recovery Master";

/* Ensure Supabase auth is always present */
(function(){
  try {
    const key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4cGtxd2NtdnRxdnViaWJicmVrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMzk4NjQsImV4cCI6MjA5MjgxNTg2NH0.afiaSFqkRFEXW5nPQVRXKZcpKkS6iF3T_hTQC2P15HQ";
    localStorage.setItem("RW_SUPABASE_ANON_KEY", key);
    if(typeof state !== "undefined") {
      state.settings = state.settings || {};
      state.settings.supabaseAnonKey = key;
...
/* Friendly JSON formatter for any AI/parts response that accidentally returns raw JSON */
function rwdRecoveryFriendlyAnswer(input){
  try {
    let data = input;
    ...
    if(!data || typeof data !== "object") return input;

    const part = data.part_query || data.part_request || data.part || data.query || data.lookup?.part_query || "";
    const source = data.source || data.lookup?.source || data.route || data.function || "Rolling Wrench AI";
    const status = data.status || data.lookup?.status || data.rpc_status || "";

    let out = "";
    if(part) out += `# Parts Master Result — ${part}\n\n`;
    else out += `# Rolling Wrench AI Result\n\n`;

    out += `## Status\n- **Source:** ${source}\n`;
    if(status) out += `- **Status:** ${status}\n`;
    if(data.error || data.message) out += `- **Message:** ${data.error || data.message}\n`;
    out += "\n";

    const crosses = data.crosses || data.cross_references || data.crossReferences || data.lookup?.crosses || [];
    if(Array.isArray(crosses) && crosses.length){
      out += "## Cross References\n";
      crosses.forEach(x => {
        if(typeof x === "string") out += `- ${x}\n`;
        else out += `- **${x.brand || x.make || x.name || "Cross"}:** ${x.part_number || x.partNumber || x.number || x.value || ""}\n`;
      });
      out += "\n";
    } else {
      out += "## Result\n- Request completed, but no verified cross-reference list was returned.\n\n";
    }

    out += "## Verify Before Ordering\n- VIN / ESN\n- Old part label\n- Dimensions\n- Application\n- Supplier catalog\n";
    return out;
  } catch(e) {
    return input;
  }
...
/* Wrap common answer extraction if present */
if(typeof rw95Answer === "function" && !window.__rwdRecoveryWrappedAnswer){
  window.__rwdRecoveryWrappedAnswer = true;
  const oldRw95Answer = rw95Answer;
  window.rw95Answer = function(data){
    try { return rwdRecoveryFriendlyAnswer(oldRw95Answer(data)); }
    catch(e) { return rwdRecoveryFriendlyAnswer(data); }
  };
  try { rw95Answer = window.rw95Answer; } catch(e) {}
}


/* ===== RWD V11 STABLE FIX — BUSINESS FLOW STABILIZER ===== */
window.RWD_STABLE_VERSION = "V11 Stable Fix";
window.RWD_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4cGtxd2NtdnViaWJicmVrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMzk4NjQsImV4cCI6MjA5MjgxNTg2NH0.afiaSFqkRFEXW5nPQVRXKZcpKkS6iF3T_hTQC2P15HQ";

(function rwdStableBoot(){
  try {
    localStorage.setItem("RW_SUPABASE_ANON_KEY", window.RWD_SUPABASE_ANON_KEY);
    if(typeof state !== "undefined"){
      state.settings = state.settings || {};
      state.settings.supabaseAnonKey = window.RWD_SUPABASE_ANON_KEY;
      state.quotes = Array.isArray(state.quotes) ? state.quotes : [];
      state.invoices = Array.isArray(state.invoices) ? state.invoices : [];
      state.customers = Array.isArray(state.customers) ? state.customers : [];
      state.truck = state.truck || {unit:"No Active Truck",vin:"NONE",customer:"",engine:"Cummins X15"};
      if(typeof saveState === "function") saveState();
    }
  } catch(e) {}
})();

function rwdStableHeaders(){
  return {"Content-Type":"application/json","apikey":window.RWD_SUPABASE_ANON_KEY,"Authorization":"Bearer "+window.RWD_SUPABASE_ANON_KEY};
}
async function rwdStableFn(name, payload){
  const res = await fetch("https://uxpkqwcmvtqvubibbrek.supabase.co/functions/v1/" + name, {
    method:"POST", headers:rwdStableHeaders(), body:JSON.stringify(payload || {})
  });
  const txt = await res.text();
  let data; try { data = JSON.parse(txt); } catch(e) { data = {text:txt}; }
  if(!res.ok){
    const msg = data.error || data.message || data.details || txt || (name+" failed");
    throw new Error(typeof msg === "string" ? msg : JSON.stringify(msg));
  }
  return data;
}
function rwdStableErr(e){
  if(!e) return "Unknown error";
  if(typeof e === "string") return e;
  if(e.message) return e.message;
  try { return JSON.stringify(e); } catch(x) { return String(e); }
}
function rwdStableMoney(n){ return "$" + Number(n||0).toFixed(2); }
function rwdStableId(prefix){ return prefix + "-" + Date.now() + "-" + Math.random().toString(36).slice(2,7); }
function rwdStableTruck(){ try { return state.truck || {}; } catch(e){ return {}; } }
function rwdStableCustomer(){ const t=rwdStableTruck(); return t.customer || "Add customer"; }
function rwdStableTruckName(){ const t=rwdStableTruck(); return t.unit || t.vin || "No Active Truck"; }
function rwdStableEngine(){ const t=rwdStableTruck(); return t.engine || "Cummins X15"; }

function rwdStableParsePreview(){
  const text = document.body.innerText || "";
  const isInvoice = /INVOICE PREVIEW|Total Due|SEND TO INVOICES/i.test(text);
  const totalMatch = text.match(/(?:Estimated Total|Total Due)\s*\$?([0-9,]+(?:\.\d{2})?)/i);
  const laborMatch = text.match(/Labor\s+([0-9.]+)\s*hrs\s*@\s*\$?([0-9.]+)/i);
  const serviceMatch = text.match(/Service Call\s*\$?([0-9,]+(?:\.\d{2})?)/i);
  const partsMatch = text.match(/Parts\s*\/\s*Materials[\s\S]*?\$?([0-9,]+(?:\.\d{2})?)/i);
  const suppliesMatch = text.match(/Supplies\s*\/\s*Fees\s*\$?([0-9,]+(?:\.\d{2})?)/i);
  const workMatch = text.match(/Work:\s*([^\n]+)/i);
  const customerMatch = text.match(/Customer:\s*([^\n]+)/i);
  const truckMatch = text.match(/Truck:\s*([^\n]+)/i);
  const clean = v => Number(String(v||"0").replace(/,/g,"")) || 0;
  const hours = laborMatch ? clean(laborMatch[1]) : 0;
  const rate = laborMatch ? clean(laborMatch[2]) : Number((state&&state.settings&&state.settings.laborRate)||135);
  const service = serviceMatch ? clean(serviceMatch[1]) : Number((state&&state.settings&&state.settings.serviceCall)||250);
  const parts = partsMatch ? clean(partsMatch[1]) : 0;
  const supplies = suppliesMatch ? clean(suppliesMatch[1]) : 25;
  const total = totalMatch ? clean(totalMatch[1]) : (hours*rate+service+parts+supplies);
  return {
    id:rwdStableId(isInvoice ? "RWI" : "RWQ"),
    type:isInvoice ? "invoice" : "quote",
    status:isInvoice ? "Unpaid" : "Draft",
    date:new Date().toLocaleString(),
    customer:(customerMatch && customerMatch[1] ? customerMatch[1] : rwdStableCustomer()).trim(),
    truck:(truckMatch && truckMatch[1] ? truckMatch[1] : rwdStableTruckName()).trim(),
    engine:rwdStableEngine(),
    work:(workMatch && workMatch[1] ? workMatch[1] : (isInvoice ? "Invoice repair work" : "Quote repair work")).trim(),
    hours:hours, rate:rate, service:service, call:service, parts:parts, supplies:supplies, total:total,
    source:"preview_capture"
  };
}
function rwdStableSaveQuoteFromPreview(){
  try {
    const q = rwdStableParsePreview();
    q.type="quote"; q.status=q.status||"Draft";
    state.quotes = Array.isArray(state.quotes) ? state.quotes : [];
    state.quotes.unshift(q);
    if(typeof saveState === "function") saveState();
    if(typeof toast === "function") toast("Quote saved");
    if(typeof setRoute === "function") setRoute("quotes"); else location.hash="#quotes";
  } catch(e) { alert("Quote save failed: " + rwdStableErr(e)); }
}
function rwdStableSaveInvoiceFromPreview(){
  try {
    const inv = rwdStableParsePreview();
    inv.type="invoice"; inv.status=inv.status||"Unpaid"; inv.invoiceId=inv.invoiceId||rwdStableId("RWI");
    state.invoices = Array.isArray(state.invoices) ? state.invoices : [];
    state.invoices.unshift(inv);
    if(typeof saveState === "function") saveState();
    if(typeof toast === "function") toast("Invoice saved");
    if(typeof setRoute === "function") setRoute("invoices"); else location.hash="#invoices";
  } catch(e) { alert("Invoice save failed: " + rwdStableErr(e)); }
}

function rwdStableQuotePaper(q,label){
  q=q||{};
  const labor = Number(q.hours||0)*Number(q.rate||0);
  const total = Number(q.total || (labor + Number(q.service||q.call||0) + Number(q.parts||0) + Number(q.supplies||0)));
  return '<section class="v86-paper rwd-stable-paper"><h2>ROLLING WRENCH DIESEL</h2><p>260-502-6222 • Mobile Diesel Repair</p><hr><h2>'+(label || (q.type==="invoice"?"INVOICE":"QUOTE"))+'</h2><p>'+String(q.date||new Date().toLocaleString())+'</p><p><b>Customer:</b> '+String(q.customer||"Add customer")+'</p><p><b>Truck:</b> '+String(q.truck||"No Active Truck")+'</p><p><b>Engine:</b> '+String(q.engine||rwdStableEngine())+'</p><p><b>Work:</b> '+String(q.work||q.desc||"Repair work")+'</p><div class="v86-line"><span>Labor</span><span>'+Number(q.hours||0)+' hrs @ '+rwdStableMoney(q.rate||0)+'</span><span>'+rwdStableMoney(labor)+'</span></div><div class="v86-line"><span>Service Call</span><span></span><span>'+rwdStableMoney(q.service||q.call||0)+'</span></div><div class="v86-line"><span>Parts / Materials</span><span></span><span>'+rwdStableMoney(q.parts||0)+'</span></div><div class="v86-line"><span>Supplies / Fees</span><span></span><span>'+rwdStableMoney(q.supplies||0)+'</span></div><div class="total">'+(q.type==="invoice"?"Total Due":"Estimated Total")+' '+rwdStableMoney(total)+'</div><div class="disclaimer"><b>Note:</b> Review before sending. Price may vary due to added labor, hidden damage, parts availability, taxes/fees, or approved extra work.</div></section>';
}

function rwdStableApprovalLink(q){
  const payload = btoa(unescape(encodeURIComponent(JSON.stringify(q))));
  return location.origin + location.pathname + "#rwdApprove?q=" + encodeURIComponent(payload);
}
function rwdStableRenderApproval(){
  if(!location.hash.startsWith("#rwdApprove")) return false;
  const params = new URLSearchParams(location.hash.split("?")[1] || "");
  let q=null;
  try { q = JSON.parse(decodeURIComponent(escape(atob(params.get("q"))))); } catch(e) {}
  const app = document.getElementById("app") || document.body;
  if(!q){
    app.innerHTML = '<main class="screen"><section class="card error"><h2>Quote Not Found</h2><p>This link is missing quote data. Ask Rolling Wrench Diesel to resend it.</p></section></main>';
    return true;
  }
  app.innerHTML = '<main class="screen"><section class="card orange"><h2>'+(q.type==="invoice"?"Invoice Approval":"Quote Approval")+'</h2></section>'+rwdStableQuotePaper(q,q.type==="invoice"?"INVOICE":"QUOTE")+'<section class="card orange"><h3>Customer / Driver Signature</h3><label>Printed Name</label><input id="rwdStableSigner" placeholder="Customer / driver name"><canvas id="rwdStablePad" class="sigpad" style="background:white;width:100%;height:190px;border-radius:16px"></canvas><div class="row"><button class="primary" id="rwdStableApproveBtn">Approve & Sign</button><button class="bad" id="rwdStableDeclineBtn">Decline</button></div></section></main>';
  rwdStableSetupSig(q);
  return true;
}
function rwdStableSetupSig(q){
  const c=document.getElementById("rwdStablePad"); if(!c) return;
  const ctx=c.getContext("2d"); let down=false,last=null,sig="";
  function fit(){ const r=c.getBoundingClientRect(); c.width=Math.max(320,r.width*devicePixelRatio); c.height=190*devicePixelRatio; ctx.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0); ctx.lineWidth=3; ctx.strokeStyle="#111"; ctx.lineCap="round"; }
  setTimeout(fit,50);
  function p(e){ const r=c.getBoundingClientRect(),t=e.touches?e.touches[0]:e; return {x:t.clientX-r.left,y:t.clientY-r.top}; }
  function start(e){ e.preventDefault(); down=true; last=p(e); }
  function move(e){ if(!down)return; e.preventDefault(); const pt=p(e); ctx.beginPath(); ctx.moveTo(last.x,last.y); ctx.lineTo(pt.x,pt.y); ctx.stroke(); last=pt; sig=c.toDataURL("image/png"); }
  function stop(){ down=false; }
  c.addEventListener("mousedown",start); c.addEventListener("mousemove",move); c.addEventListener("mouseup",stop); c.addEventListener("mouseleave",stop);
  c.addEventListener("touchstart",start,{passive:false}); c.addEventListener("touchmove",move,{passive:false}); c.addEventListener("touchend",stop);
  document.getElementById("rwdStableApproveBtn").onclick=function(){
    q.status="Approved"; q.signature={name:document.getElementById("rwdStableSigner").value, data:sig, date:new Date().toLocaleString()};
    app.innerHTML = '<main class="screen"><section class="card orange"><h2>Approved</h2><p>Thank you. Rolling Wrench Diesel received the approval.</p></section>'+rwdStableQuotePaper(q,q.type==="invoice"?"SIGNED INVOICE":"SIGNED QUOTE")+'</main>';
  };
  document.getElementById("rwdStableDeclineBtn").onclick=function(){ app.innerHTML = '<main class="screen"><section class="card"><h2>Declined</h2><p>Rolling Wrench Diesel will follow up.</p></section></main>'; };
}

function rwdStableCleanPart(q){
  const m=String(q||"").toUpperCase().match(/\b[A-Z]*\d[A-Z0-9-]{4,}\b/);
  return m ? m[0].replace(/[^A-Z0-9]/g,"") : String(q||"").replace(/cross[- ]?reference|part number|lookup|verify/ig,"").trim();
}
function rwdStableLooksParts(q){
  const s=String(q||"").toLowerCase();
  return /\b\d{5,}\b/.test(s)||s.includes("cross reference")||s.includes("xref")||s.includes("water pump")||s.includes("part number")||s.includes("fleetguard")||s.includes("baldwin")||s.includes("wix")||s.includes("donaldson")||s.includes("napa");
}
function rwdStableFormatParts(data, original){
  if(typeof data === "string") return data;
  const d=data||{}, look=d.lookup||d.data||d.result||d;
  const part = rwdStableCleanPart(original || d.part_query || d.query || look.part_request || "");
  let out = "# Parts Master Result — " + (part||"Part Request") + "\\n\\n";
  out += "## Status\\n- **Source:** " + (d.source || look.source || "Oracle Parts Search") + "\\n";
  if(d.error || look.error) out += "- **Message:** " + (d.error || look.error) + "\\n";
  out += "\\n";
  const crosses=d.crosses||d.cross_references||d.crossReferences||look.crosses||look.cross_references||[];
  if(Array.isArray(crosses) && crosses.length){
    out += "## Cross References\\n";
    crosses.forEach(x=>{
      if(typeof x==="string") out += "- " + x + "\\n";
...
async function rwdStablePartsSearch(q){
  const part_query = rwdStableCleanPart(q);
  const payload={part_query, query:part_query, prompt:part_query, raw_engine_text:q, vin_text:(state&&state.truck&&state.truck.vin&&state.truck.vin!=="NONE")?state.truck.vin:null, context:{truck:(state&&state.truck)||{}, settings:(state&&state.settings)||{}}};
  try { return rwdStableFormatParts(await rwdStableFn("oracle-parts-search", payload), q); }
  catch(e) {
    try { return rwdStableFormatParts(await rwdStableFn("verify-part-web", payload), q); }
    catch(e2) { throw new Error("Parts search unavailable: " + rwdStableErr(e2)); }
  }
}

const rwdStableOldAsk = typeof rw92AskBackend === "function" ? rw92AskBackend : null;
async function rwdStableAsk(prompt, files){
  try {
    if(!files && rwdStableLooksParts(prompt)) return await rwdStablePartsSearch(prompt);
    if(rwdStableOldAsk) return await rwdStableOldAsk(prompt, files);
    return "Tell me what you need.";
  } catch(e) { return "Backend error: " + rwdStableErr(e); }
}
try { window.rw92AskBackend = rwdStableAsk; rw92AskBackend = rwdStableAsk; } catch(e) {}

document.addEventListener("click", function(e){
  const btn=e.target.closest("button, .button, [role='button']");
  if(!btn) return;
  const t=String(btn.textContent||btn.value||"").toLowerCase();
  if(t.includes("send to quotes")){
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
    rwdStableSaveQuoteFromPreview(); return false;
  }
  if(t.includes("send to invoices")){
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
    rwdStableSaveInvoiceFromPreview(); return false;
  }
  if(t.includes("customer link") || t.includes("approval link") || t.includes("create link")){
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation();
    const q=rwdStableParsePreview();
    const link=rwdStableApprovalLink(q);
    navigator.clipboard && navigator.clipboard.writeText(link).catch(()=>null);
